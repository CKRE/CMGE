...
def model_config():
    ...
    parser.add_argument('--output_dir', type=str, default='./node_mask_output')
    ...
    parser.add_argument('--explain_epoch', type=int, default=400)
...
def main():
    ...
    if config.explainer:
        ...
        for idx, batch_data in tqdm(enumerate(test_data_loader)):
            # node_feat_mask, edge_mask, graph = model(batch_data, epochs=config.explain_epoch, explain=config.explainer)
            sample_id = batch_data['sample_id'][0]
            dir_path = os.path.join(config.output_dir, sample_id)
            if not os.path.exists(dir_path):
                os.mkdir(dir_path)

            node_feat_mask, edge_mask, node_mask, graph, pred_label = explainer.explain_node(batch_data)
            plt.figure(idx)
            ax, G, indices = explainer.visualize_subgraph(graph, edge_mask, node_mask, label=batch_data['type'][0])
