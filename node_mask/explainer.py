...
class GNNExplainer(nn.Module):
    ...
    def explain_node(self, input_data):
        ...
        optimizer = torch.optim.Adam([self.node_mask], lr=0.01)
        for epoch in range(1, self.epochs + 1):
            optimizer.zero_grad()
            graph.edata['edge_mask'] = self.edge_mask
            graph.ndata['node_mask'] = self.node_mask
            # graph.ndata['h'] = sub_feature * self.node_feat_mask.view(1, -1).sigmoid()
            graph.ndata['h'] = torch.cat([(sub_feature[i] * self.node_mask[i]).unsqueeze(0) for i in range(sub_feature.size(0))], dim=0)
            graph_output = self.model.GAT(graph.ndata['h'], graph, explain=True)
            log_logits = F.softmax(self.model.fc_g(graph_output[-1:]), dim=1)
        ...
        return node_feat_mask, edge_mask, node_mask, graph, pred_label
    ...
    def __set_masks__(self, graph, init='normal'):
        ...
        std = nn.init.calculate_gain('relu') * sqrt(2.0 / (2 * N))
        self.node_mask = nn.Parameter(torch.randn(N) * std)
    ...
    def visualize_subgraph(self, graph, edge_mask, node_mask, threshold=None, label=None, **kwargs):
        assert edge_mask.size(0) == graph.number_of_edges()
        assert node_mask.size(0) == graph.number_of_nodes()
        
        if threshold is not None:
            edge_mask = (edge_mask >= threshold).to(torch.float) 
            node_mask = (node_mask >= threshold).to(torch.float)

        values, indices = node_mask.topk(5)
        
        graph = graph.cpu()
        graph.edata['edge_mask'] = edge_mask.cpu()
        graph.ndata['node_mask'] = node_mask.cpu()
        graph = dgl.node_subgraph(graph, indices.cpu())
